--[[
    ...
    local Window = ViloniX.CreateWindow({
        ...
        KeySystem = true,                          -- true/false
        Keys      = { "KEY-1234", "DEV-9999" },
        KeyNote   = "Discord: discord.gg/xyz",
        ToggleKey = Enum.KeyCode.RightShift,       -- бинд GUI
    })
    ...
    -- Методы Window:
    Window:Destroy()                           -- выключить всё и удалить GUI
    Window:SetBind(Enum.KeyCode.Insert)        -- сменить бинд
    Window:Toggle()                            -- открыть / закрыть
]]
...
local function ShowKeySystem(cfg, onSuccess)
    ...
    -- Accent line сверху
    New("Frame", {
        Size=UDim2.new(1,0,0,3), BackgroundColor3=THEME,
    ...
    local ConfirmBtn = New("TextButton", {
        ...
        BackgroundColor3=THEME, Text="Подтвердить",
    ...
    local function TryKey()
        ...
        if valid then
            ...
            Tween(Panel, 0.35, { Size=UDim2.new(0,0,0,0) }, Enum.EasingStyle.Quart, Enum.EasingDirection.In)
            task.wait(0.4)
            pcall(function() KeyBlur:Destroy() end)
            pcall(function() Gui:Destroy() end)
            task.wait(0.05) -- небольшая пауза перед показом основного GUI
            onSuccess()
        else
            Status.TextColor3 = Color3.fromRGB(255,75,75)
            Status.Text       = "✗  Неверный ключ."
            -- Shake
            local origPos = UDim2.new(0,20,0,96)
            for _, dx in ipairs({7,-7,5,-5,3,0}) do
                Tween(InputWrap, 0.04, { Position=UDim2.new(0,20+dx,0,96) })
                task.wait(0.04)
            end
            InputWrap.Position = origPos
        end
    end
...
function ViloniX.CreateWindow(options)
    ...
    local Window = {
        ...
        _tabs        = {},
        _currentPage = nil,
        _isOpen      = false,       -- ← отслеживаем состояние вручную
    }
    ...
    -- ── Main Frame ────────────────────────────
    -- ВАЖНО: начинаем невидимым, но с нормальным размером
    -- Видимость контролируем только через _isOpen
    ...
    -- Разделители
    local function DivLine(pos, sz)
        New("Frame", { BackgroundColor3=cfg.Border, BorderSizePixel=0, Position=pos, Size=sz }, Main)
    end
    DivLine(UDim2.new(0,0,0,62),   UDim2.new(1,0,0,1))
    DivLine(UDim2.new(0,192,0,62), UDim2.new(0,1,1,-62))
    DivLine(UDim2.new(0,0,1,-112), UDim2.new(0,192,0,1))
    ...
    New("TextLabel", {
        Text=cfg.SubTitle, Position=UDim2.new(1,-170,0,0), Size=UDim2.new(0,105,1,0),
        TextColor3=Color3.fromRGB(70,70,70), Font=MF, TextSize=12,
    ...
    local BindBtn = New("TextButton", {
        Size=UDim2.new(0,88,0,28), Position=UDim2.new(1,-220,0.5,-14),
        BackgroundColor3=Color3.fromRGB(18,18,18),
        Text=cfg.ToggleKey.Name, TextColor3=THEME,
    ...
    local FloatBtn = New("TextButton", {
        ...
        Visible=true,  -- всегда видна
    }, ScreenGui)
    Round(FloatBtn, 22)
    New("UIStroke", { Color=THEME, Thickness=1.5 }, FloatBtn)

    -- ─────────────────────────────────────────
    --  ОТКРЫТЬ / ЗАКРЫТЬ  (надёжная версия)
    -- ─────────────────────────────────────────
    ...
    local function OpenGUI()
        ...
        -- Показываем мгновенно, затем анимируем размер
        Main.Size    = UDim2.new(0, 0, 0, 0)
        Main.Visible = true
        Blur.Enabled = true

        tweenOpen = Tween(Main, 0.4, { Size=UDim2.new(0,W,0,H) })
    ...
    local function CloseGUI()
        ...
        tweenClose = Tween(Main, 0.3, { Size=UDim2.new(0,0,0,0) },
            Enum.EasingStyle.Quart, Enum.EasingDirection.In)
        Tween(Blur, 0.3, { Size=0 },
            Enum.EasingStyle.Quart, Enum.EasingDirection.In)
        tweenClose.Completed:Once(function()
            if not Window._isOpen then  -- не открыли снова пока закрывался
                Main.Visible = false
                Blur.Enabled = false
            end
        end)
    ...
    local function DoDestroy()
        ...
        -- Выключаем все toggle callback'и (esp, aimbot и т.д.)
        ...
        -- Анимация и удаление
        Tween(Main, 0.35, { Size=UDim2.new(0,0,0,0) },
            Enum.EasingStyle.Quart, Enum.EasingDirection.In)
    ...
    -- Двойное нажатие для подтверждения Destroy
    local destroyStep = 0
    DestroyBtn.MouseButton1Click:Connect(function()
        destroyStep += 1
        if destroyStep == 1 then
            DestroyBtn.Text = "?"
            Tween(DestroyBtn, 0.15, { BackgroundColor3=Color3.fromRGB(220,100,30) })
    ...
    local function ElementAPI(page)
        ...
        function api:AddToggle(label, default, callback)
            ...
            -- Регистрация для автовыключения при Destroy
            table.insert(Window._toggleFns, function(val)
                if active ~= val then active = val Refresh(true) end
            end)
        ...
        function api:AddSlider(label, opts, callback)
            ...
            local t1 = Track.InputBegan:Connect(function(i)
                if i.UserInputType==Enum.UserInputType.MouseButton1 then sliding=true fromMouse(i.Position.X) end
            end)
            local t2 = UserInputService.InputChanged:Connect(function(i)
                if sliding and i.UserInputType==Enum.UserInputType.MouseMovement then fromMouse(i.Position.X) end
            end)
            local t3 = UserInputService.InputEnded:Connect(function(i)
                if i.UserInputType==Enum.UserInputType.MouseButton1 then sliding=false end
            end)
            table.insert(Window._connections, t1)
            table.insert(Window._connections, t2)
            table.insert(Window._connections, t3)
        ...
        function api:AddColorPicker(label, default, callback)
            ...
            local function UCol()
                ...
                Swatch.BackgroundColor3=color SV.BackgroundColor3=Color3.fromHSV(H,1,1)
                SC.Position=UDim2.new(S,0,1-V,0) HC.Position=UDim2.new(H,-2,0,-2)
                callback(color)
            end
    ...
    -- ─────────────────────────────────────────
    --  ЗАПУСК — ключевое исправление здесь
    --  task.defer гарантирует что GUI полностью
    --  построен до первого показа
    -- ─────────────────────────────────────────

    if cfg.KeySystem then
        -- Показываем ключ-систему, после успеха открываем основной GUI
        task.defer(function()
        ...
        -- Без ключа — открываем сразу, но после инициализации
        task.defer(function()
            OpenGUI()
        end)
    end
